  ...
  &:hover {
   ...
   print"";
    ...
    print"<th class='text-center'>NAME</th>";
     ...
     // print"<th></th>";
    print" <th><a href='classlist.php?subcode=$subcode&date=$date&classno=$i'> <span style='  margin-left: -13px;  color: #070;'class=' glyphicon glyphicon-plus'></span></a> </th>";
    print"</tr></thead>";
    ...
    while($i<=$count)
    {
         $query2="Select date from Attend where hours=$i group by subcode_fk having subcode_fk='$subcode'; ";
         $dates=mysqli_query($dbHandle,$query2);                   
         $rowdate=mysqli_fetch_array($dates,MYSQL_ASSOC);
        $day=$rowdate['date'];      
        print "<tr class='clickable-row' data-href='viewattendance.php?subcode=$subcode&date=$day&classno=$i'>";
        print"<td class='text-center'>$i</td>";
            

        print"<td id='sub'  class='text-center'>Class $i</td>";
         include('db_connection.php');
       
        print"<td style='' class='text-center'>$day</td>";
      print"<td> <a href='editattendance.php?subcode=$subcode&date=$day&classno=$i'>
