    ...
    <body>
        ...
        <!--navbar-->
          ...
          <div class="wrapper">
            ...
            <!-- content goes here -->
			<form action="insertsubjects.php" method="POST">
                <div class="form-group" >
                <label for="name">Name</label>
                <input type="text"  name="name" class="form-control" id="name" value="<?php  print $_GET['name'];?>">
              </div>
               <div class="form-group" >
                <label for="name">Usn</label>
                <input type="text"  name="usn" class="form-control" id="name" value="<?php  print $_GET['usn'];?>">
              </div>
            
                 <?php  
                   include('db_connection.php');
                   $usn=$_GET['usn'];
                   $q="select sec from Students where usn='$usn';";
                    $sq=mysqli_query($dbHandle,$q);
                    $r=mysqli_fetch_array($sq,MYSQL_ASSOC);
                    $sec=$r['sec'];
                    print"<p style='color:black;'>Select Subjects:</p>";
                    print" <div class='funkyradio'>";
                   
                    
                     if($sec==1){
                 include('db_connection.php');
                    $query="SELECT sub_name,subcode FROM Subjects WHERE subcode LIKE '1%';";
                    $s=mysqli_query($dbHandle,$query);
                    $x=mysqli_num_rows($s);
                    $i=1;
                
                    while($rowsub=mysqli_fetch_array($s,MYSQL_ASSOC))
                    {
                        $subname=$rowsub['sub_name'];
                        $subcode=$rowsub['subcode'];
                        $a=substr("$subcode",1,10);
                         print "<div class='funkyradio-info'>";
                         if($i<=8)
                    print"<input checked class='single-checkbox' type='checkbox' name='subcode[]' value='$subcode' id='checkbox$i' />";
                    else
                                        print"<input  class='single-checkbox' type='checkbox' name='subcode[]' value='$subcode' id='checkbox$i' />";

                    print" <label for='checkbox$i'>$a - $subname</label>";
                    print"</div>";
                    $i=$i+1;
                    }
                     }
                     if($sec==2){
                 include('db_connection.php');
                    $query="SELECT sub_name,subcode FROM Subjects WHERE subcode LIKE '2%';";
                    $s=mysqli_query($dbHandle,$query);
                    $x=mysqli_num_rows($s);
                    $i=1;
                    ...
                    <script>
                        ...
                        if ($('input[type=checkbox]:checked').length > 8) {
                                ...
                                alert("You can select only 8 subjects!");
                        }
                        })
                    </script>
